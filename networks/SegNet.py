...
class SegNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Stage 4d
        x4d = F.max_unpool2d(x51d, id4, kernel_size=2, stride=2, output_size=x4_size) # 计算一个部分逆
...
def get_segnet_model(n_class=1):
    model = SegNet(classes=n_class)
    return model
...
def test():
    ...
    net = get_segnet_model().to(device)
