...
class FinalPatchExpand_X4(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        >>> a = torch.randn(12, 64, 64, 32)
        >>> rearrange(a, 'b h w (p1 p2 c)-> b (h p1) (w p2) c', p1=4, p2=4, c=32//(4**2)).shape
        torch.Size([12, 256, 256, 2])
        """
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.num_features = int(embed_dim * 2 ** (self.num_layers - 1)) # 96*2^{4-1}=768
        self.num_features_up = int(embed_dim * 2) # 96*2=192
        self.mlp_ratio = mlp_ratio
        ...
        if self.final_upsample == "expand_first":
            ...
            self.output = nn.Sequential(
                nn.Conv2d(in_channels=embed_dim,out_channels=64,kernel_size=1,bias=False),
